...
class ContactList extends StatefulWidget {
  final List<dynamic> contacts = [
    ...
    },
    {
      ...
      "location": {
        "street": "7457 rue du bât-d'argent",
        "city": "préverenges",
        "state": "graubünden",
        "postcode": 5610
      },
  ...
  createState() => new _ContactList();
}

class _ContactList extends State<ContactList> with AfterLayoutMixin<ContactList> {
  var startTime;
  var endTime;
  ...
  void afterFirstLayout(BuildContext context) {
    ...
    Duration duration = this.endTime.difference(this.startTime);
    print("Difference (us): ${duration.inMicroseconds}");
  }
  ...
  Widget build(BuildContext context) {
    ...
    return new Scaffold(
      appBar: new AppBar(
        title: new Text('Flutter Performance on Lists'),
      ),
      body: new ListView.builder(
        ...
        itemBuilder: (context, index) {
          return new ListTile(
            title: new Text(
              this.widget.contacts[index]['name']['title'] + ' ' + this.widget.contacts[index]['name']['first'] + ' ' + this.widget.contacts[index]['name']['last'],
              style: new TextStyle(
                fontSize: 18.0,
                fontWeight: FontWeight.bold,
              ),
            ),
            subtitle: new Text(this.widget.contacts[index]['phone']),
          );
        },
      ),
    );
